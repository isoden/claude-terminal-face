...
| `thinking` | 考える顔（視線が斜め上、口を閉じる）                  | Claude Code hooks（調査/プランニング中）                       |
| `working`  | 集中顔（目を細める、口は真一文字）                    | Claude Code hooks（実装中）                                    |
| `done`     | ドヤ顔（片目を細める＋口角を大きく上げる）            | Claude Code hooks（応答完了）                                  |
